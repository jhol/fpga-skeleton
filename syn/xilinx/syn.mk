...
include $(ROOT_DIR)/prj/xilinx/prj.mk

PROJECT_DIR=$(ROOT_DIR)/prj/xilinx

$(PROJECT_NAME)_top.ngc: ${RTL_DIR}/*.v $(PROJECT_DIR)/$(PROJECT_NAME)_top.xst \
	$(PROJECT_DIR)/$(PROJECT_NAME)_top.prj
	make prepare
	xst -ifn "$(PROJECT_DIR)/$(PROJECT_NAME)_top.xst"

$(PROJECT_NAME).ngd: $(ROOT_DIR)/$(CONSTRAINT_FILE) $(PROJECT_NAME)_top.ngc
	ngdbuild -p ${DEVICE_PART} -uc $(ROOT_DIR)/$(CONSTRAINT_FILE) -aul \
